...
namespace MusicXml.Tools.Converter
{
    public static class Alignment
    {
        ...
        private class Beat : List<Part>
        {
            public Beat() : base()
            { }

            public Beat(IEnumerable<Part> collection) : base(collection)
            {
            }
        }

        private class Measure : List<Beat>
        {
        }
            ...
            int beats)
        {
            ...
            Alignment.divisions = divisions;
            Alignment.beats = beats;
            ...
            for (int beatId = 0; beatId < Alignment.beats; beatId++)
            {
                Beat beat = measure[beatId];
            ...
            Beat combinedBeats = CombineBeats(ref measure);
        ...
        private static void CheckJianpuRules(ref Beat beat)
        {
            for (int i = 0; i < partNum; i++)
            {
                Part part = beat[i];
        ...
        private static void InsertHeadSpace(ref Beat beat)
        {
            for (int i = 0; i < partNum; i++)
                beat[i].InsertRange(0, Enumerable.Repeat(string.Empty, WIDTH_BETWEEN_BEATS));
        }

        private static void InitializeSpace(ref Beat beat)
        {
        ...
        private static void InsertAlters(ref Beat beat)
        {
            for (int partId = 0; partId < partNum; partId++)
            {
        ...
        private static void InsertDotsInBeat(ref Beat beat)
        {
            for (int partId = 0; partId < partNum; partId++)
            {
                Part part = beat[partId];
        ...
        private static Beat CombineBeats(ref Measure measure)
        {
            Beat combinedBeats = new Beat(measure.Last());
        ...
        private static void InsertDotsInMeasure(ref Beat combinedBeats)
        {
            #region for '.'
            for (int partId = 0; partId < partNum; partId++)
            {
        ...
        private static void DetermineTypes(ref Beat combinedBeats)
        {
        ...
        private static void RemoveLastSpace(ref Beat combinedBeats)
        {
            for (int i = 0; i < partNum; i++)
            {
                var part = combinedBeats[i];
