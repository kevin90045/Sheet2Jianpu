...
namespace MusicXml
{
	public static class MusicXmlParser
	{
		...
		private static Note GetNote(XmlNode noteNode)
		{
            ...
            note.TimeModification = GetTimeModification(noteNode);
        ...
        private static TimeModification GetTimeModification(XmlNode noteNode)
        {
            var time_modification = new TimeModification();
            var time_modificationNode = noteNode.SelectSingleNode("time-modification");
            if (time_modificationNode != null)
            {
                var actual_notesNode = time_modificationNode.SelectSingleNode("actual-notes");
                if (actual_notesNode != null)
                    time_modification.ActualNotes = int.Parse(actual_notesNode.InnerText);

                var normal_notesNode = time_modificationNode.SelectSingleNode("normal-notes");
                if (normal_notesNode != null)
                    time_modification.NormalNotes = int.Parse(normal_notesNode.InnerText);
            }
